# SmartNews Platform Monorepo

SmartNews CoreをHeadless CMSとして、Creators、Bizの3つのアプリケーションで構成されるモノレポです。

> **⚠️ Claude Code を使用する場合**: 必ず最初に [`CLAUDE_CODE_GUIDE.md`](./CLAUDE_CODE_GUIDE.md) を読み込んでください。モノレポ構造、Git連携、Vercelデプロイに関する重要な情報が記載されています。

## 📁 プロジェクト構成

```
smartnews-platform/
├── apps/
│   ├── core-cms/        # SmartNews Core (管理CMS)
│   ├── creators/        # SmartNews for Creators (公開サイト)
│   └── biz/             # SmartNews Biz (公開サイト)
├── packages/
│   ├── database/        # Supabase型定義とクライアント
│   ├── ui/              # 共通UIコンポーネント
│   └── config/          # 共通設定
└── package.json
```

## 🚀 開始方法

### 1. 依存関係のインストール

```bash
pnpm install
```

### 2. 環境変数の設定

各アプリケーションで `.env.local` を作成してください。

**apps/core-cms/.env.local**
```
NEXT_PUBLIC_SUPABASE_URL=your_supabase_url
NEXT_PUBLIC_SUPABASE_ANON_KEY=your_supabase_anon_key
```

### 3. 開発サーバーの起動

```bash
# すべてのアプリケーションを起動
pnpm dev

# 特定のアプリケーションのみ起動
pnpm --filter @smartnews/core-cms dev
pnpm --filter @smartnews/creators dev
pnpm --filter @smartnews/biz dev
```

## 🔧 利用可能なコマンド

| コマンド | 説明 |
|---------|------|
| `pnpm dev` | すべてのアプリケーションを開発モードで起動 |
| `pnpm build` | すべてのアプリケーションをビルド |
| `pnpm lint` | すべてのアプリケーションでlintを実行 |
| `pnpm type-check` | すべてのアプリケーションで型チェックを実行 |

## 🌐 ポート番号

| アプリケーション | ポート |
|----------------|--------|
| core-cms | 3000 |
| creators | 3001 |
| biz | 3002 |

## 📦 共通パッケージ

### @smartnews/database
Supabaseの型定義とクライアント関数を提供。

```typescript
import { createBrowserClient, type Post } from '@smartnews/database';
```

### @smartnews/ui
共通UIコンポーネントを提供（今後実装予定）。

```typescript
import { ArticleRenderer } from '@smartnews/ui';
```

## 🏗️ アーキテクチャ

```
┌─────────────────┐
│  SmartNews Core │ ← 管理者/クリエイターが記事を作成
│     (CMS)       │
└────────┬────────┘
         │
         ├─── distribution_channels テーブルに配信先を記録
         │
         v
┌─────────────────┐
│    Supabase     │ ← 中央データベース
│    (Database)   │
└────────┬────────┘
         │
    ┌────┴────┐
    │         │
    v         v
┌─────────┐ ┌─────────┐
│Creators │ │   Biz   │ ← 各サイトがSupabaseから記事を取得
│  Web    │ │  Web    │
└─────────┘ └─────────┘
```

## 🔄 ワークフロー

1. **Core CMS**: 管理者・クリエイターが記事を作成し、配信先（Creators/Biz/Newsletter）を選択
2. **Database**: Supabaseに記事データと配信設定を保存
3. **Creators/Biz**: それぞれのサイトが配信設定に基づいて記事を取得・表示

## 📝 開発ガイドライン

- 共通の型定義は `packages/database` に配置
- 再利用可能なUIコンポーネントは `packages/ui` に配置
- 各アプリケーションの独自機能は `apps/{app-name}` 内で完結させる
